    ...
    public:
        ...
        maze(int xD, int yD) {
            ...
            for (int x = 0; x < xDim; x++) {
                for (int y = 0; y < yDim; y++) {
                    if (y>0) {
                        graph[mp(x,y)].push_back(mp(x,y-1));
                    }
                    if (y < yDim-1) {
                        graph[mp(x,y)].push_back(mp(x,y+1));
                    }
                    if (x > 0) {
                        graph[mp(x,y)].push_back(mp(x-1,y));
                    }
                    if (x < xDim-1) {
                        graph[mp(x,y)].push_back(mp(x+1,y));
                    }
                    shuffle(begin(graph[mp(x,y)]), end(graph[mp(x,y)]), default_random_engine { random_device {}()});
                }
            }
        }

        void mazeMakerWilson() {
            ...
            pair<int, int> end = mp(xDim/2, yDim/2);
